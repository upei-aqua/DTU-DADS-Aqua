ConstpMat <- function(dist,
  ...
  fillCat <- function(distVec, distCat) {
    ...
    for (i in 1:length(distCat)) dTmp <- dTmp + as.numeric(distVec >= distCat[i])
  ...
  list(
    ...
    fillProb <<- function(pMatTmp, distprobA, tmpID) {
      ...
      # I do not normalize, because in such a case some shipments from Bornholm
      # will end up in the sea :-) TH
      return(pMatTmp)
    },
    ...
    addIDs <- function(IDs) {
      ...
      pMatIDs[newCol] <<- IDs
      pMat[, newCol]  <<- fillProb(pMat[, newCol, drop = FALSE],
                                   distprobA,
                                   pMatIDs[newCol])
      usedCol         <<- usedCol + n
      return(newCol)
    ...
    get = function(origins) {
      ...
      if (reduce) {
        ...
        if (verbose) {
          cat("In pMat$get: Reducing to:", length(found)," of",origins,"\n")
        }
