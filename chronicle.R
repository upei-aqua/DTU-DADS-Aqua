...
constructChronicle <-
  function(herdfile    = warning("No herdfile specified when chronicle Initialized"),
           typesfile   = warning("No typesfile specified when Chronicle Initialized"),
           dumpTrigger = 1e5) {
 ...
 ######################## INITIALIZE AND ADD DATA:
  ...
  if(exists("runID")) runID <<- get("runID")
  if(is.null(runID)||(runID == "")) runID <<- round(runif(1,10000,99999))
  ...
  zero <- function() {
    ...
    number    <<- number + 1
    ## Reset the number of lines in the chronicle
    prevEntry <<- 0
  ...
  list(
    ...
    ## - so keep it fast!
    addEntry = function(itn              = stop(noItr),
                        state            = stop(noState),
                        newInfection     = 0,
                        time             = gTime,
                        changedHerdsList = stop(noHerdsList),
                        SourceHerds      = NA,
                        HerdSize         = stop(noHerdSize),
                        HerdTypes        = stop(noHerdTypes)) {

      if(is.na(changedHerdsList[1])) stop("CHL problem in chronicle")

      ## dump chronicle if the number of lines is greater than trigger
      if(dumpSaves && prevEntry>=trigger) dump()
      ...
      if(traceBack){ ## Record if doing trace backs
        ...
        if (newInfection > 0) { ## but only if a newInfection
          ...
          for (i in 1:length(SourceHerds)) {
            traceList[[ charSource[i] ]]  <<- rbind(traceList[[charSource[i]]],
                                                    matrix(c(time,changedHerdsList[i],
                                                             newInfection),
                                                    ncol = 3))
          }##EndOf for ( i in ...)
    ...
    wrapUp = function(savefile = FALSE) {
      ...
      if (!(savefile)) savefile <- paste(runID, "ASF.bix", sep = "-")
