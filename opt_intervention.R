##### OPT_INTERVENTION library version 0.20
  ...
  list(
    ...
    day  = switch(Zone, {
      function() {
        ...
        Temp <- (aHerd$inProtZone) &
                !aHerd$Diagnosed &
                !aHerd$status %in% c(5,6) &
                aHerd$ToSurvDead < gTime
        ...
        SurvToday    <- which((aHerd$inProtZone) &
                              !aHerd$Diagnosed &
                              (aHerd$status <= 4) &
                              aHerd$ToSurvDead == gTime)
        ...
        if (length(SurvToday) > 0) {
          ...
          if (length(InfToSurvay) > 0) {
            ...
            numOfSamples <-  ifelse(Deaths <= aHerd$NumSampDead[SurvToday],
                                    Deaths,
                                    aHerd$NumSampDead[SurvToday])
            probDet <-
              (1 - (1 - (numOfSamples[Tmp] / (Deaths[Tmp] - ((DeadOfASF - 1) / 2)))) ^ DeadOfASF)
      ...
      function() {
        ### On daily basis, check which herds to be set for surveillance of dead animals.
        Temp <- (aHerd$inProtZone|aHerd$inSurZone) &
                !aHerd$Diagnosed & aHerd$status <= 4 &
                 aHerd$ToSurvDead < gTime
        ...
        ### Check which herds to be survayed today, given that they still are in a zone.
        SurvToday <- which((aHerd$inProtZone | aHerd$inSurZone) &
                           !aHerd$Diagnosed &
                           !aHerd$status %in% c(5,6) &
                            aHerd$ToSurvDead == gTime)
        ...
        if (gTime >= (startCulling + gDaysUntilBaseline)) {
          ...
          if (length(infHerdNums) > 0) {
            ...
            if (sum(Rdepops) > 0) {
              ...
              if (Detailed) {
                PreEmpMatOut <<-
                  rbind(PreEmpMatOut, cbind(iteration, gTime, which(Rdepops)))
                if (dim(PreEmpMatOut)[1] >= DumpData) {
                  # NAME here will be exactly the same as that in the
                  # initialization file so no worries; no overwriting will happen
                  # ;-) (TH)
