#### ASF model interface version 0.20
...
## test         This feature is only included for use .Rd files
...
ASF <- function(args = ASFoptions(), test = FALSE) {
  ...
  environment(SurvHerds)          <- environment()
  ...
  for (i in 1:length(controlFunctions)) {
    controlList[[i]] <- as.list(controlFunctions[[i]])
    controlFun[[i]]  <- eval(controlList[[i]][[1]])
    environment(controlFun[[i]]) <- environment()
  }
  ...
  for (i in 1:length(interventionFunctions)) {
    InterList[[i]] <- as.list(interventionFunctions[[i]])
    InterFun[[i]]  <- eval(InterList[[i]][[1]])
    environment(InterFun[[i]]) <- environment()
  }
